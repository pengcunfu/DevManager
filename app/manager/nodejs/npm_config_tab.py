...
class NpmMirrorTestThread(QThread):
    ...
    def __init__(self, configurator: NpmConfigurator, timeout: int = 5):
        super().__init__()
...
class NpmConfigTab(QWidget):
    """NPM 配置页面"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.configurator = NpmConfigurator()
    ...
    def create_config_panel(self) -> QWidget:
        ...
        self.npm_version_label = QLabel("NPM 版本: 加载中...")

        current_layout.addWidget(self.npm_version_label)
        ...
        # 测试当前源按钮
        test_current_btn = QPushButton("测试当前源")
        test_current_btn.clicked.connect(self.test_current_mirror)
        button_layout.addWidget(test_current_btn, 0, 1)
    ...
    def populate_mirror_combo(self):
        ...
        # 按推荐顺序排序
        recommended_order = ['npmmirror', 'tencent', 'huawei', 'ustc', 'official']
    ...
    def load_current_config(self):
        ...
        try:
            # 检查NPM是否安装
            if not self.configurator.check_npm_installed():
                self.npm_version_label.setText("NPM 版本: 未安装")
                self.current_mirror_label.setText("当前镜像源: NPM未安装")
                self.current_url_label.setText("镜像地址: -")
                return
            ...
            try:
                ...
                if result.returncode == 0:
                    self.npm_version_label.setText(f"NPM 版本: {result.stdout.strip()}")
                else:
                    self.npm_version_label.setText("NPM 版本: 获取失败")
            except Exception:
                self.npm_version_label.setText("NPM 版本: 获取失败")

            # 获取当前镜像源
            registry = self.configurator.get_current_registry()
            if registry:
                self.current_mirror_label.setText(f"当前镜像源: {registry}")

                # 查找匹配的镜像源
                for key, mirror in self.configurator.MIRRORS.items():
                    if mirror['url'].rstrip('/') in registry or registry.rstrip('/') in mirror['url'].rstrip('/'):
                        self.current_url_label.setText(f"镜像名称: {mirror['name']}")
                        # 更新下拉框选择
                    ...
                    self.current_url_label.setText(f"镜像名称: 未知源")
            else:
                self.current_mirror_label.setText("当前镜像源: 官方源（默认）")
                self.current_url_label.setText("镜像地址: https://registry.npmjs.org/")
    ...
    def configure_mirror(self, mirror_key=None):
        ...
        # 检查NPM是否安装
        if not self.configurator.check_npm_installed():
            QMessageBox.warning(self, "错误", "NPM未安装，请先安装Node.js和NPM")
            return
    ...
    def test_current_mirror(self):
        ...
        try:
            registry = self.configurator.get_current_registry()
            if not registry:
                QMessageBox.warning(self, "警告", "无法获取当前镜像源配置")
                return
            ...
            # 测试速度
            self.status_label.setText("正在测试当前镜像源...")
            speed = self.configurator.test_mirror_speed(current_key, current_mirror, 10)
            ...
            if speed is not None:
                QMessageBox.information(
                    ...
                    "测试结果",
                    f"当前镜像源响应速度:\n\n"
                    f"镜像源: {current_mirror['name']}\n"
                    f"地址: {current_mirror['url']}\n"
                    f"响应时间: {speed * 1000:.0f} ms"
                )
            else:
                QMessageBox.warning(
                    self,
            ...
            QMessageBox.critical(self, "错误", f"测试当前镜像源时出错: {str(e)}")
    ...
    def start_test(self):
        ...
        # 创建并启动测试线程
        self.test_thread = NpmMirrorTestThread(self.configurator)
